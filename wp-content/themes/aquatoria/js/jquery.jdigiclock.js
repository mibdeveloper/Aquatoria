...
(function($) {
    $.fn.extend({
        ...
        jdigiclock: function(options) {
            ...
            var defaults = {
                clockImagesPath: path + '/img/digiclock/clock/',
                weatherImagesPath: path + '/img/digiclock/weather/',
                am_pm: false,
            ...
            return this.each(function() {
                ...
                html    += '<p id="left_arrow"><img src="'+ path +'/img/digiclock/icon_left.png" alt="Alternate Text" /></p>';
                html    += '<p id="right_arrow"><img src="'+ path +'/img/digiclock/icon_right.png" alt="Alternate Text" /></p>';
                ...
                var panel_pos = ($('#plugin_container>div').length - 1)*310;

                var next = function() {
                    $('#right_arrow').unbind('click', next);
                    $('#plugin_container > div').filter(function(i) {
                        $(this).animate({'left': (i * 310) - 310 + 'px'}, 240, function() {
                            if (i == 0) {
                ...
                var prev = function() {
                    ...
                    $('#plugin_container > div:last').prependTo('#plugin_container').css({'left':'-310px'});
                    $('#plugin_container > div').filter(function(i) {
                        $(this).animate({'left': i * 310 + 'px'}, 240, function() {
                            if (i != 0) {
                                $(this).appendTo('#plugin_container').css({'left':panel_pos + 'px'});
                            }else
...
el.currDate = date_ru + ',&nbsp;' + now.getDate() + '&nbsp;' + month_ru;
// time update
...
$.fn.getWeather = function(el) {
    ...
    url = path + '/proxy.php?location=' + el.weatherLocationCode + '&metric=' + el.weatherMetric;
    $.getJSON(url, function(data) {
        ...
        var weather = '<div id="local"><p class="city">Евпатория</p><p>' + data.curr_text + '</p></div>';
...
var curr_for = curr_temp + '<p class="high_low">' + data.day_htemp + '&deg;&nbsp;/&nbsp;' + data.day_ltemp + '&deg;</p>';
curr_for    += '<p class="city">Евпатория</p>';
curr_for    += '<p class="text">' + data.curr_text + '</p>';
el.find('#current').css('background','url(' + el.weatherImagesPath + data.curr_icon + '.png) 50% 0 no-repeat').append(curr_for);
...
el.find('#forecast_container').append('<div id="update"><img src="'+ path + '/img/digiclock/refresh_01.png" alt="reload" title="reload" id="reload" />' + el.timeUpdate + '</div>');
